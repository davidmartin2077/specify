...
1. 先比较下面每条样本的上下文是否真的不同。
2. 再看不同上下文是否足以产生不同训练价值。
3. 如果只是换标题/平台/泛泛场景，结论倾向 `删掉`、`退回重写` 或 `降权`。
4. 如果确实构成同文不同语境对照，结论可写 `保留对照`。
...
- 上下文：
  - title：课程/教程内容
  - description：无
  - parent_comment：讨论学习或技术问题
  - reply_chain：上下文支持普通解释
  - time：2026-03-06
  - topic：平台黑话/自我审查
- 关键 reasoning：
  - supporting_evidence：无
