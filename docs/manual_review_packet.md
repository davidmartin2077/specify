...
3. `docs/context_policy_decision.md`
4. `docs/external_safety_import_review_sample.md`
5. `docs/context_audit_codebook.md`
6. `data/raw/combined_candidates_binary_preview.jsonl`
7. `data/raw/external_safety_binary_preview.jsonl`
8. `data/eval/risk_test_preview.jsonl`
9. `data/eval/normal_test_preview.jsonl`
...
本轮开始不再单独精修 `parent_comment/reply_chain` 板块。旧数据里如果风险主要靠“像，但别展开”“别说像谁”“懂就别打全称”“缩写就行”这类合成回复链成立，直接按模板污染、退回重写或降权处理。
...
MEME_EXPAND_0044_MEDIUM：D03，退回重写；风险主要靠合成回复链成立，不采信。
```
...
- 必须靠标题、发布时间、真实事件背景或话题才能判断风险。
- 去掉上下文后，风险等级会明显下降。

不再鼓励因为合成 `parent_comment/reply_chain` 而保留 `context_required=true`。这些字段除非来自真实原始数据，否则默认不作为强证据。
