...
| `C02` | `contextual_required` | 文本单看较模糊，需要标题、时间、话题、真实事件背景等支撑当前风险解释。 | 保留上下文依赖，但检查上下文是否真实、自然、够用；合成回复链默认不作强证据。 |
...
最新策略：不再单独精修 `parent_comment/reply_chain` 板块。旧数据里如果风险主要靠“像，但别展开”“别说像谁”“懂就别打全称”“缩写就行”等合成回复链成立，优先按模板污染、退回重写或降权处理。详见 `docs/context_policy_decision.md`。
