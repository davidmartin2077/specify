#!/usr/bin/env python3
"""Compare capability coverage before and after the phase-3 first wave."""
...
PHASE3 = ROOT / "data/raw/phase3_first_wave_candidates.jsonl"
OUTPUT_JSON = ROOT / "data/processed/phase3_coverage_delta.json"
OUTPUT_MD = ROOT / "docs/phase3_coverage_delta.md"
...
def build_report() -> dict[str, Any]:
    ...
    phase2 = coverage.load_jsonl(PHASE2)
    phase3 = coverage.load_jsonl(PHASE3)
    before = coverage.sample_summary(processed + phase2)
    after = coverage.sample_summary(processed + phase2 + phase3)
    return {
        "inputs": {
            "processed": len(processed),
            "phase2": len(phase2),
            "phase3_first_wave": len(phase3),
        },
        "before": before,
        "after": after,
        "delta": {
...
def main() -> int:
    ...
    print(f"Wrote phase-3 coverage delta to {OUTPUT_JSON}")
    print(f"Wrote readable phase-3 coverage delta to {OUTPUT_MD}")
